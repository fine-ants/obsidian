...
**staleTime과 gcTime 설정 이전에 TanStack Query는 실제로 inactive 되어있던 쿼리의 데이터를 fetch해오는가? 새로운 문자를 입력하고 쿼리 요청을 보낼 때 이전의 분리되어있는 검색어들의 쿼리 키들은 inactive한 상태로 바뀐다. gcTime이 끝나기전에 같은 `inactive되어있던 문자와 같은 문자로 검색했을 때
