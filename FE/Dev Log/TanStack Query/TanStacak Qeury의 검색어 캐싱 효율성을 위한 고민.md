...
위와 같은 고민이 생겼던 이유
- `staleTime`을 검증하는 시간보다 길게 설정해놓아서 inactive한 쿼리키의 데이터를 가져와서 사용할 때도 staleTime이 지나지 않아서 fresh하다고 처리되는 부분을 새로운 요청을 한다고 오해함. `fresh` 상태는 
- staleTime을 3초정도로 줄여놓고 처음 '삼성전자'를 입력하는 시간동안만 첫 쿼리키들은 fresh한 값으로 처리되고, '삼성전자'의 마지막 'ㅏ'를 입력하면서 쿼리 요청을 보낸 시점으로부터 3초가 지나면 전부 stale한 값으로 정의됨. 이후 '삼성전자'를 또 다시 입력하면 모든 분리된 요청에서 이전의 입력되었던 inactvie한 쿼리키들의 의 값을 fetch해옴.
