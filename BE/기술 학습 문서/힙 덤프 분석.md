...
ps -ef | grep java
```
- 도커 컨테이너로 실행하는 경우에 다음과 같이 PID를 조회할 수 있습니다.
```shell
...
예를 들어 위 예시에서 나온 fineAnts_app spring 서버의 힙 덤프 파일을 생성해봅니다.
```shell
jmap -dump:format=b,file=testdump.hprof 3133132
```


> [!NOTE] jmap package 설치
> ```shell
> 
> ```
