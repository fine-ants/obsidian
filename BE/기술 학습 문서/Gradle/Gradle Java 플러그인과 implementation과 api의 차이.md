...
Java-Library는 Java 라이브러리에 대한 특정한 규칙(예: 프로덕션 코드는 src/main/java에 위치)을 제공해서 Java 플러그인의 기능을 확장하는 플러그인입니다. 이 플러그인을 사용할 때는 Java 플러그인에 의해 제공되는 모든 소스 세트(main, test), 테스크, 설정이 별도의 설정 없이 사용 가능합니다.
