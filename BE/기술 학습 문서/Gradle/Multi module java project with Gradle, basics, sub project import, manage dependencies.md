...
- [[#루트 프로젝트 생성 및 모듈 생성|루트 프로젝트 생성 및 모듈 생성]]
	- [[#루트 프로젝트 생성 및 모듈 생성#gradlebasics 루트 프로젝트 생성|gradlebasics 루트 프로젝트 생성]]
	- [[#루트 프로젝트 생성 및 모듈 생성#sub-project-1 모듈 생성|sub-project-1 모듈 생성]]
	- [[#루트 프로젝트 생성 및 모듈 생성#sub-project-2 모듈 생성|sub-project-2 모듈 생성]]
- [[#루트 프로젝트 - project 설정|루트 프로젝트 - project 설정]]
	- [[#루트 프로젝트 - project 설정#루트 프로젝트 - subprojects 설정|루트 프로젝트 - subprojects 설정]]
	- [[#루트 프로젝트 - project 설정#루트 프로젝트 - allprojects 설정|루트 프로젝트 - allprojects 설정]]
- [[#common 모듈 생성하기|common 모듈 생성하기]]
- [[#모듈간 의존성 추가하기|모듈간 의존성 추가하기]]
	- [[#모듈간 의존성 추가하기#common 모듈에 guava 의존성을 추가하기|common 모듈에 guava 의존성을 추가하기]]
- [[#외부 파일을 이용한 의존성 관리하는 방법|외부 파일을 이용한 의존성 관리하는 방법]]
...
![그림](refImg/Pasted%20image%2020251129153705.png)

sub-project-1 모듈에 소스 코드 디렉토리 생성
![그림](refImg/Pasted%20image%2020251129153946.png)

sub-project-1 모듈, 패키지 생성
![그림](refImg/Pasted%20image%2020251129154459.png)

subprojectone 패키지에 클래스 생성
![그림](refImg/Pasted%20image%2020251129154629.png)


`build.gradle` 파일 생성
해당 디렉토리에 `build.gradle` 파일을 생성한다는 의미는 해당 디렉토리를 독립적인 Gradle 모듈로 정의하고 그 모듈의 **빌드 방식과 설정을 명시**하겠다는 의미입니다.
![그림](refImg/Pasted%20image%2020251129154711.png)
...
![그림](refImg/Pasted%20image%2020251201125226.png) ![그림](refImg/Pasted%20image%2020251201125253.png)


### sub-project-2 모듈 생성
sub-project-1 모듈과 같이 디렉토리를 직접 생성하는 방법이 아닌 Intellij Module 메뉴를 통해서 생성해봅니다.
![그림](refImg/Pasted%20image%2020251129155343.png)

생성창에서 이름과 JDk 버전, 상위 모듈 또는 프로젝트를 설정할 수 있습니다.
![그림](refImg/Pasted%20image%2020251129155455.png)

sub-project-2 모듈 생성 확인
![그림](refImg/Pasted%20image%2020251129155534.png)
...
![그림](refImg/Pasted%20image%2020251129160512.png)

![그림](refImg/Pasted%20image%2020251129160613.png)
![그림](refImg/Pasted%20image%2020251129160708.png)
...
![그림](refImg/Pasted%20image%2020251129162906.png)
...
![그림](refImg/Pasted%20image%2020251129163027.png)
...
![그림](refImg/Pasted%20image%2020251129163933.png)
...
![그림](refImg/Pasted%20image%2020251129164132.png)
...
common 모듈 생성
![그림](refImg/Pasted%20image%2020251130122407.png)

패키지 생성
![그림](refImg/Pasted%20image%2020251130122835.png)
...
![그림](refImg/Pasted%20image%2020251201102757.png)
...
![그림](refImg/Pasted%20image%2020251201103402.png)
...
![그림](refImg/Pasted%20image%2020251201105609.png)
...
![그림](refImg/Pasted%20image%2020251201105725.png)
...
![그림](refImg/Pasted%20image%2020251201142127.png)
...
![그림](refImg/Pasted%20image%2020251201111126.png)
...
![그림](refImg/Pasted%20image%2020251201111420.png)

api 의존성 설정으로 변경후 Gradle을 리로드한 다음에 sub-project-1 모듈의 ModuleOneUser 클래스를 보면 ImmutableMap 클래스를 참조할 수 있습니다.
![그림](refImg/Pasted%20image%2020251201142436.png)

이번에는 Gradle Build하여 빌드가 되는지 테스트해봅니다.
![그림](refImg/Pasted%20image%2020251201111518.png)


## 외부 파일을 이용한 의존성 관리하는 방법
다음 의존성 설정은 루트 프로젝트의 의존성 설정입니다. 그중에서 lombok 라이브러리(1.18.22)를 compileOnly, annotationProcessor 의존성 설정한 것을 볼수 있습니다. 해당 설정에서 "org.projectlombok:1.18.22" 라이브러리 및 버전 설정을 2곳에 각각에 설정하였습니다. 만약에 버전이 변경된다면 2곳 모두를 수정해야 할 것입니다. 즉, 코드 중복의 문제점이 발생할 수 있습니다.
![그림](refImg/Pasted%20image%2020251201143118.png)

위와 같이 코드 중복의 문제점을 해결하기 위해서 외부의 gradle 파일을 이용하여 의존성을 관리할 수 있습니다. 별도의 gradle 파일을 생성하기 위해서 다음과 같이 루트 프로젝트에 `dependencies.gradle` 파일을 생성합니다.
![그림](refImg/Pasted%20image%2020251201112556.png)

루트 프로젝트에 있는 모든 의존성을 dependencis.gradle 파일로 이동시키고 ext 설정을 이용해서 다음과 같이 설정합니다. 또한 common 모듈에 있는 guava 의존성 또한 가져와 설정하도록 합니다.
![그림](refImg/Pasted%20image%2020251201113101.png)

루트 프로젝트의 build.gradle에서 `dependencis.gradle` 파일을 참조하여 의존성 설정을 적용합니다.
![그림](refImg/Pasted%20image%2020251201115912.png)
![그림](refImg/Pasted%20image%2020251201114358.png)

위와 같이 설정한 다음에 build 하여 설정이 적용되는지 확인해봅니다. 실행 결과를 보면 정상적으로 빌드된 것을 볼수 있습니다.
![그림](refImg/Pasted%20image%2020251201114638.png)

sub-project-1 모듈에서 junitJupiterApi가 정상적으로 동작하는지 테스트 클래스를 생성하고 실행해봅니다.
![그림](refImg/Pasted%20image%2020251201120005.png)

실행 결과를 보면 정상적으로 테스트가 성공한 것을 볼수 있습니다.
```shell
./gradlew test
```
![그림](refImg/Pasted%20image%2020251201120020.png)

## References
- https://youtu.be/pSKY3-K9_qc?si=1V8G94YUMscrVGkk