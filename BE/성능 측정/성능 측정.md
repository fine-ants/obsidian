...
### SSE 응답 시간 성능 측정
테스트할 sse 수행할 api는 다음과 같습니다.
- api : http://localhost:8080/api/portfolio/1/holdings/realtime
- sse 연결시간 : 30초
- 데이터 푸시받는 간격 : 5초
제가 원하는 것은 10명의 가상 사용자가 각각 api를 요청하여 sse 데이터를 푸시받을 것입니다.
위 성능 측정을 수행하기 위해서 Linux CLI를 이용할 것입니다.
```sh
rm *.log
...
  (timeout 60s curl -N -s -H "Cookie: $COOKIE" http://localhost:8080/api/portfolio/1/holdings/realtime | ts '[%Y-%m-%d %H:%M:%.3S]' > user_$i.log &)
