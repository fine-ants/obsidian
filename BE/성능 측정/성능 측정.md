...
한국투자증권 Open API와 Redis를 통한 종목 정보(현재가, 종가 등) 갱신 기능 개발
도입 배경
- 사용자들은 각자 자신만의 포트폴리오에 종목들을 등록합니다. 이 중 몇몇 종목들은 중복되는 경우가 있습니다. 사용자들이 포트폴리오 페이지에 접속하게 되면 클라이언트는 서버에게 종목에 대한 가격 정보를 요청합니다. 서버는 요청을 받은 다음에 한국투자증권 서버에 질의한 다음에 종목의 가격 정보를 전달합니다. 하지만 중복된 종목들에 대하여 일일히 한국투자증권 서버에 질의하는 것은 비효율적이라고 판단하였습니다. 그래서 질의한 가격 정보를 어딘가에 저장해야 한다고 판단하였습니다.
    
- 종목 가격 정보에 대한 중복 질의 문제를  해결하기 위해서 질의한 종목 가격 정보를 Redis 저장소에 저장하여 불필요한 중복 질의를 줄였습니다.
사용 이유
- 질의한 정보를 저장하기 위해서 Redis 저장소를 선택하였습니다. 로컬 캐시가 아닌 Redis 저장소를 선택한 이유는 여러개의 서버로 운용시 각각의 서버가 종목 정보를 캐시에 저장되어 있지 않으면 한국투자증권 서버에 질의하기 때문입니다. 이러한 상황을 방지하기 위해서 글로벌 저장소인 Redis를 선택하게 되었습니다.
