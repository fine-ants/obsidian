...
2. 각 종목에 대하여 만료시간이 현재시간보다 이후인지 검사합니다.
3. 만약 만료시간이 현재시간보다 이후의 시간이라면 넘어갑니다.
4. 만약 만료시간이 현재시간보다 이전 시간이라면 결과 컬렉션에서 저장합니다.
5. 1~4번 과정을 수행하여 나온 결과 컬렉션에 들어있는 종목들을 StockPriceRepository에서 제거합니다.
6. 제거한 종목에 대하여 종목 실시간 체결가 구독을 해제 메시지를 전송합니다.
