...
1. 사용자가 포트폴리오 종목 조회(SSE)를 요청합니다.
2. 서버는 사용자의 포트폴리오에 등록된 종목들의 티커심볼들을 저장소(StockPriceRepository)에 넣습니다.
3. 어떤 한 서비스는 한국투자증권의 실시간 종목 체결가에 웹소켓 연결합니다.
4. 한국투자증권으로부터 실시간 체결가에 대한 정보를 받습니다.
5. 정보를 받으면 종목에 따른 현재가를 레디스 저장소에 저장합니다.
6. 2~5번 과정을 다른 서비스 또는 모듈이 수행하는 동안 별도로 포트폴리오 종목 조회 API를 처리하여 사용자에게 SSE 응답합니다.