...
- 하나의 포트폴리오에 20개를 넘는 종목들이 등록되어 있고 실시간 포트폴리오 종목 조회 요청시 최대 20개까지의 실시간 체결가를 받을 수 있습니다. 나머지 80개의 종목에 대해서는 종목의 저장소에 넣지 않고 REST API 방식인 종목의 현재가를 조회하여 응답해야 합니다.
	...
	3. 종목 실시간 체결가 저장소에 저장된 종목들은 StockPriceDispatcher에 의해서 StockPriceService에 종목 실시간 체결가 구독을 요청합니다.
	4. 
	5. 종목 실시간 체결가 저장소에 저장되지 않은 종목들에 대해서는 REST API 방식을 이용하여 종목의 현재가를 조회한 다음에 현재가를 레디스에 저장합니다.
