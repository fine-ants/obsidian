...
#### 처리
- 종목을 관리하는 StockPriceRepository에서는 우선순위가 낮은 종목에 대해서 제거하고 실패한 종목을 추가해야 한다
	- 종목의 우선순위 기준 : 가장 먼저 저장된 종목이 가장 먼저 구독 해제된다 (FIFO)
- FineAnts 서버는 저장소에 있는 종목 중 우선순위가 낮은 종목을 한국투자증권 웹소켓 서버의 구독을 해제해야 합니다.
- FineAnts 서버는 실패한 종목을 대상으로 다시 웹소켓 서버의 구독을 등록해야 합니다.
