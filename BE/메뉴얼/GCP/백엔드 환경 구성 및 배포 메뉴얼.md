...
- [[#SSH 키 생성 및 인스턴스 접속|SSH 키 생성 및 인스턴스 접속]]
	- [[#SSH 키 생성 및 인스턴스 접속#SSH 키 생성하기|SSH 키 생성하기]]
	- [[#SSH 키 생성 및 인스턴스 접속#생성된 SSH 키를 GCP 콘솔을 통해 서버에 등록하기|생성된 SSH 키를 GCP 콘솔을 통해 서버에 등록하기]]
	- [[#SSH 키 생성 및 인스턴스 접속#로컬 SSH 설정 파일에 접속 정보 추가|로컬 SSH 설정 파일에 접속 정보 추가]]
- [[#백엔드 서버 환경 구성|백엔드 서버 환경 구성]]
	- [[#백엔드 서버 환경 구성#패키지 매니저 업데이트|패키지 매니저 업데이트]]
	- [[#백엔드 서버 환경 구성#도커 프로세스 설치|도커 프로세스 설치]]
	- [[#백엔드 서버 환경 구성#docker-compose 설치|docker-compose 설치]]
- [[#swap-memory 설정|swap-memory 설정]]
- [[#MySQL 데이터 GCP Storage 버킷에 백업하기|MySQL 데이터 GCP Storage 버킷에 백업하기]]
		- [[#docker-compose 설치#1. VM에 gcloud와 gsutil 설치 확인|1. VM에 gcloud와 gsutil 설치 확인]]
		- [[#docker-compose 설치#2. GCP 인증(서비스 계정 권장)|2. GCP 인증(서비스 계정 권장)]]
		- [[#docker-compose 설치#3. MySQL 데이터 백업 스크립트 작성|3. MySQL 데이터 백업 스크립트 작성]]
		- [[#docker-compose 설치#4. CronJob 등록|4. CronJob 등록]]
	- [[#MySQL 데이터 GCP Storage 버킷에 백업하기#트러블 슈팅: GCS 버킷 파일 업로드 403 문제|트러블 슈팅: GCS 버킷 파일 업로드 403 문제]]
...
if sudo tar -cf - . | sudo gzip > "$BACKUP_FILE"; then
    ...
    if gsutil cp "$BACKUP_FILE" gs://gcp-fineants-bucket/production/mysql/; then
        ...
        echo "Failed to upload backup to GCP Storage."
        exit 2
    fi
else
...
./backup_mysql_data.sh
```
![](BE/메뉴얼/GCP/refImg/Pasted%20image%2020250916152813.png)
