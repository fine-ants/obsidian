...
Compute Engine default service account 계정에서 fineants-share-backend 계정으로 변경하고자 합니다.
![](BE/메뉴얼/GCP/refImg/Pasted%20image%2020250916124734.png)

gcloud CLI에서 서비스 계정 변경
```shell
gcloud compute instances set-service-account fineants-vm \
  --zone=asia-northeast3-a \
  --service-account=fineants-share-backend@fineants-470303.iam.gserviceaccount.com \
...
sudo tar -cf - . | sudo gzip > "$BACKUP_FILE"  
  
# GCP Storage 업로드  
gsutil cp "$BACKUP_FILE" gs://gcp-fineants-bucket/production/mysql/  
...
CronJob 등록
```shell
0 0 * * * /home/fineants/backup_mysql_data.sh
```
