...
1. Route53 서비스로 이동하여 호스팅 영역 메뉴로 이동합니다. 그리고 호스팅 영역 생성 버튼을 클릭합니다.
![[Pasted image 20231226141153.png]]

2. 호스팅 영역을 생성하기 위한 정보를 입력합니다.
![[Pasted image 20231226141325.png]]

3. 생성 결과를 확인합니다. 다음 그림에서 보면 NS 유형의 레코드의 값들을 가비아에서 구입하 도메인의 네임서버에 등록해주어야 합니다.
...
3. 인스턴스에 따른 로그 그룹 정보를 입력합니다.
![[Pasted image 20231227171835.png]]
...
3. 퍼블릭 인증서 요청 페이지에서 도메인 이름, 검증 방법, 키 알고리즘 옵션을 다음과 같이 선택한 다음에 하단의 요청 버튼을 클릭합니다.
	...
	- 단순히 `*.fineants.co`만 추가하면 `fineants.co`에서 사용할 수 없는 인증서가 되기 때문에 2차 도메인(`fineants.co`)과 3차 도메인(`*.fineant`) 모두에서 사용하려면 다음과 같이 도메인을 추가해야 합니다.
![[Pasted image 20241209135408.png]]

4. 인증서 생성을 확인합니다.
	- 상태가 검증 대기중인 상태인데 이는 아직 DNS 검증이 진행되지 않았기 때문입니다.
![[Pasted image 20241209135113.png]]

5. 인증서 상세 화면으로 들어가서 다음과 같이 "Route 53에서 레코드 생성" 버튼을 클릭하여 레코드를 생성합니다.
![[Pasted image 20241209140538.png]]

6. 2개의 도메인을 선택하고 레코드 생성 버튼을 클릭합니다.
![[Pasted image 20241209140631.png]]

7. Route53에서 보면 다음과 같이 레코드가 생성된 것을 볼수 있습니다.
![[Pasted image 20241209141130.png]]

### 로드 밸런싱 생성
생성한 SSL 인증서를 ELB와 같은 다른 AWS 서비스와 연결되어야 합니다.
1. EC2 서비스의 로드 밸런싱 - 로드 밸런서 메뉴를 클릭합니다.
![[Pasted image 20241209141554.png]]
...
대상 그룹을 생성하기 위해서 다음과 같이 설정합니다.
![[Pasted image 20241209145557.png]]
![[Pasted image 20241209145613.png]]
![[Pasted image 20241209145636.png]]

대상 그룹 생성 2단계에서 대상 그룹에 포함시킬 인스턴스를 선택후 "아래에 보류 중인 것으로 포함"을 클릭합니다. 그리고 나서 대상 그룹을 생성합니다.
...
이전 단계에서 만들어 두었던 ACM 인증서를 선택합니다.
![[Pasted image 20241209151346.png]]

리스너를 등록한 다음에 로드 밸런서 생성합니다. 그리고 로드 밸런서 생성 확인합니다.
![[Pasted image 20241209151419.png]]

80번 리스너에 대해서 80번 포트로 들어오는 요청에 대하여 443 포트로 리다이렉션 하도록 설정합니다.
![[Pasted image 20241209151513.png]]

![[Pasted image 20241209151545.png]]

다음 실행 결과를 보면 80번 리스너는 443 포트로 리다이렉션 됩니다.
![[Pasted image 20241209151602.png]]

Route53 서비스로 이동하여 생성한 로드밸런서를 레코드에서 생성합니다.
![[Pasted image 20241209151905.png]]

레코드 생성을 확인하고 상태 또한 확인합니다.
![[Pasted image 20241209152019.png]]
![[Pasted image 20241209152043.png]]
