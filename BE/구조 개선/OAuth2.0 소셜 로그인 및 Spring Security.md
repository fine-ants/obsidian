...
직접 구현한 소셜 로그인 방식으로 구현시 유지보수 관점에서 어떤 어려움이 있을까? 
현재 상황에서 봤을때 유지보수 관점에서 어떤 어려움?
-> 인증 상태 유지 전략을 JWT가 아닌 세션으로 바꾸는 경우,
만약에 액세스 토큰을 가지고 프로필 정보를 조회할 때 추가적인 정보가 필요할 때 어디를 수정하는가? 프로퍼티 설정 파일에서 스코프를 변경하거나. 스코프만으로 
