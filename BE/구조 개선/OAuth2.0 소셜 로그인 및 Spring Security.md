...
### 도입 배경
Spring Security 프레임워크를 도입하게 된 이유는 다음과 같았습니다.
- 단순 인증한 사용자들이 사용하는 API만이 아닌 관리자 권한을 요구하는 API도 필요하게 되었습니다. 따라서 기존 인증 시스템만이 아닌 API별로 상세한 접근 권한을 제어하는 인가 처리 시스템이 필요하게 되었습니다.
- 
