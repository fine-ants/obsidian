...
### 기존 인증 시스템을 Spring Security 프레임워크에 맞게 재구현한 이유는 무엇인가?
Spring Security 프레임워크를 도입하면서 기존 인증 시스템을 유지한 상태에서 프레임워크의 인가 처리를 이용하기 위해서는 인증 후에 별도의 처리를 구현해야 했습니다. Spring Security에서는 인증을 수행한후에 인증 정보를 `Authentication` 객체에 넣어서 `SecurityContextHolder`에 저장합니다. `Authentication` 객체는 이후 hasRole(), `@Secured` 애노테이션와 같은 인가 처리시 사용하게 됩니다. 하지만 기존 인증 시스템을 유지하면서 인가 처리만 프레임워크를 사용하게 된다면 인증을 성공한 후에 별도의 `Authentication` 객체를 생성하고 `SecurityContextHodler`에 수동으로 저장해야 합니다.

즉, 정리하면 기존 인증 시스템을 유지하게 되면 인증 과정을 성공후에 별도로 프레임우
