...
### 도입 배경
Spring Security 프레임워크를 도입하게 된 이유는 **인가 처리 시스템** 때문이었습니다. 기존에는 단순 인증한 사용자들이 사용하는 API만으로도 충분했지만 시간이 지나면서 서버를 관리하기 위해서 관리자 권한을 요구하는 API도 필요하게 되었습니다. 따라서 기존 인증 시스템만이 아닌 API별로 상세한 접근 권한을 제어하는 인가 처리 시스템이 필요하게 되었습니다.

### 기존 인증 시스템을 Spring Security 프레임워크에 맞게 재구현한 이유는 무엇인가?
Spring Security 프레임워크를 도입하면서 기존 인증 시스템을 유지한 상태에서 프레임워크의 인가 처리를 이용하기 위해서는 인증 후에 별도의 처리를 구현해야 했습니다. 
